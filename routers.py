...
from services.category_service import create_category, delete_category
...
@api.get('/me')
async def me(db: Annotated[Session, Depends(get_db)],
             current_user: Annotated[models.User, Depends(get_current_user)]) -> schemas.User:
    return current_user


@api.patch('/me')
async def updates_user(data: schemas.UserCreateOrUpdate,
...
@api.post('/me')
async def creates_user(data: schemas.UserCreateOrUpdate,
                       db: Annotated[Session, Depends(get_db)],
                       current_user: Annotated[models.User, Depends(get_current_user)]) -> schemas.User:
...
@api.post('/categories')
async def new_category(category: schemas.CategoryCreate,
                       db: Annotated[Session, Depends(get_db)],
                       current_user: Annotated[models.User, Depends(get_current_user)]):
...
async def new_task(id: int, task: schemas.TaskCreate,
                   db: Annotated[Session, Depends(get_db)],
                   current_user: Annotated[models.User, Depends(get_current_user)]) -> schemas.Task:
...
async def get_all_tasks(id: int,
                        db: Annotated[Session, Depends(get_db)],
                        current_user: Annotated[models.User, Depends(get_current_user)]) -> list[schemas.Task]:

    return get_tasks(db, current_user, id)


@api.post('/categories/{id}/tasks/{task_id}')
async def checks_task(id: int, task_id: int,
                      db: Annotated[Session, Depends(get_db)],
                      current_user: Annotated[models.User, Depends(get_current_user)]):
...
async def finished_tasks(id: int,
                         db: Annotated[Session, Depends(get_db)],
                         current_user: Annotated[models.User, Depends(get_current_user)]) -> list[schemas.Task]:
